	...
	<div class="bunding">
	 	...
	 	<header class="top-logo">
	 		<div class="table">
	 		 ...
	 		 <p class="name">为保证您的账户安全，请验证如下信息</p>
	 	</header>
	 	<section class="center-content">
	 		...
	 		<div class="item">
	 			...
	 			<div class="auto"><input type="text" class="identity-code" name="identity-code" placeholder="请输入证件号码" v-model='identityNum' @input='checkInput'  @focus='scrollView($event)' /></div>
	 			
	 		</div>
	 		<div class="item">
	 				<div class="label"><label>手机号码</label></div>
	 				<div class="auto"><input type="number" name="phone-num" placeholder="请输入手机号码" maxlength="11" v-model='phoneNum'  @input='checkInput' @focus='scrollView($event)' /></div>
	 		
	 		</div>
	 		<div class="item">
	 		 	<div class="label"><label>图形验证码</label></div>
				<div class="img-1"><input class="img-code" type="text" name="" placeholder="图形验证码" maxlength="4" v-model='imgCode' @input='checkInput'  @focus='scrollView($event)' /></div>
				<div class="img-2">
	 		...
	 		<div class="item">
	 			...
	 			<div class="msg-1"><input class="msg-code" type="text" name="" placeholder="短信验证码"v-model='msgCode' @input='checkInput' @focus='scrollView($event)' /></div>
	 			<div class="msg-2">	
	 	...
	 	<footer class="footer">
	 		<p class="info"><img src="./images/phone.png"/>华泰证券资管客服电话 <a class="tel" href="tel:95597">95597</a></p>
	 		<button type="button" class="bund-btn" @click='bind' :class='{"unbind-btn" : unbind}'>下一步</button>
	 	...
	 	<confirm :isOpen='isUnbindOpen'  @close='closeConfirm'></confirm>
...
<script type="text/javascript">
    ...
    import { Certificates ,OpenId,clearStorage} from 'src/common/js/global';
	...
	export default{
		...
		mounted(){	
			this.openId = sessionStorage.OPNEID ;  		  
		},
		methods:{
			checkInput(){
				this.$v.$touch();
			...
			closeConfirm(code){
				...
				if(code==0){       //取消
					...
					params:{
				    	...
				    	if(response.data.flag.code==0){  //解除成功
					 ...
					 }else if(response.data.flag.code==1){  //失败
						this.infoOpen = true;
						this.isUnbindOpen = false;
			...
			bind(){
				...
				this.axios.get(Bunding.bind,{
					...
					if(response.data.flag.code==0){  //解除成功
				        ...
				        this.infoOpen = true;
				        this.info = '您输入的信息与账户绑定时所填信息不一致，解绑失败！人工服务请拨打95597！'
						//this.ecifId = response.data.ecifId;
